...
def guide_evaluate_unevaluated_abstract(request):
    ...
    if request.method == "POST":
        ...
        if thesis.status.id != STATUS_ID_ABSTRACT_WAITING_APPROVAL:
...
def guide_evaluate_unevaluated_synopsis(request):
    ...
    if request.method == "POST":
        ...
        if thesis.status.id != STATUS_ID_SYNOPSIS_WAITING_APPROVAL:
            return HttpResponse('0', content_type = 'text/plain')
...
def guide_evaluate_unevaluated_thesis(request):
    ...
    if request.method == "POST":
        ...
        if thesis.status.id != STATUS_ID_THESIS_WAITING_APPROVAL:
            return HttpResponse('0', content_type = 'text/plain')
...
def guide_send_panel_to_admin(request):
    ...
    if request.method == "POST":
        ...
        if thesis.status.id > STATUS_ID_WAITING_FOR_PANEL_APPROVAL:
            dict = {'status' : 'OK', 'message' : 'Already panel sent to admin by another guide'}
            ...
            if ThesisGuide.objects.filter(thesis = thesis, guide = guide):
                ...
                if panelmember:
                    ...
                    for referee in panelmember:
                        report = {}
                        report['filename'] = referee.feedback_without_referee_details
